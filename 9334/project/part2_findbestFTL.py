...
def random_model(fogTimeLimit,see):
    lam=9.72
    a1=0.01
    a2=0.4
    b=0.86
    ...
    def ran_arrival(lam):# random arrival_time
        
        u=np.random.uniform(0,1)
        t=-np.log(1-u)/lam
        return t

    def ran_network(v1,v2):#get random network late
    ...
    totalNumberOfJobsCompleted=0 #number of completed jobs
        ...
        if event ==1:#dep from fog without cloud
            ...
            for el in job_list_fog:
                if el[1]==min(l_min):
                    ...
                    if master_clock>500:#dayu 500 jiru
    ...
    mrt=round(meanres,4)
    return mrt,response_time


def show_tu(fogTimeLimit):
    m_mrt=[]
    see=1
    for see in range(0,50):
            mrt,response_time=random_model(fogTimeLimit,see)
            arr=np.array(response_time)
            stde=np.std(arr)
            stde=round(stde,4)
            print(see)
            m_mrt.append(mrt)
    k=st.t.interval(0.95, len(m_mrt)-1, loc=np.mean(m_mrt), scale=st.sem(m_mrt))

    plt.vlines(fogTimeLimit, k[0], k[1])

for limit in [0.10,0.11,0.12]:
    show_tu(limit)
plt.show()
